...
define([
	...
	function(declare, lang, array, html, esriBundle, on, query, Json, Deferred, aspect, all,
		...
		Extent, Point,FeatureLayer, rendererJsonUtils, Button, editUtils, FilterEditor, RelatedRecordsEditor) {
		return declare([BaseWidget, _WidgetsInTemplateMixin], {
			...
			onOpen: function() {
				var ywLayer = this.map.getLayer("YW");
				this.yLayer = ywLayer;
				this.map.removeLayer(ywLayer);
				require(['esri/layers/FeatureLayer'], lang.hitch(this, function(layerClass) {
					var layer, infoTemplate, options = {outFields:["*"]},
						layer = new layerClass('http://112.4.212.98:6080/arcgis/rest/services/KFQ/GYYD/FeatureServer/0', options);
					this.fLyer = layer;
			...
			_createRelatedRecordsEditor: function(feature) {
				...
				var loadingDomNode = html.create('div', {
			...
			onClose: function() {
				this.map.removeLayer(this.fLyer);
				this.map.addLayer(this.yLayer);
				if(this.editor) {
					this.editor.destroy();
				}
				this.editor = null;
