      ...
      postCreate: function(){
        ...
        if(this.map){
          this.own(on(document.body, 'click', lang.hitch(this, this._onDocumentBodyClick)));
            var feaLayer = new FeatureLayer(this.appConfig.featureLayer,{outFields:["*"]});
            myFeatureTable = new FeatureTable({
                featureLayer: feaLayer,
                outFields: ["YDDW", "SFGSQY", "JYDJ", "JYLX", "YDLX", "QSXZ", "SZXZ", "SZC", "CYLB",
            ...
            //单击表中数据，定位到该图形,弹出框
            // var markerSymbol = new PictureMarkerSymbol("widgets/Chart/images/locate.png", 22, 22);
            // myFeatureTable.setSelectionSymbol(markerSymbol);  //  报错
            myFeatureTable.on("row-select", function (evt) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    if (geo.attributes["OBJECTID"] == id)
                    {
                        ...
                        this.map.infoWindow.setContent(geo.getContent());
                        this.map.infoWindow.setTitle(geo.getTitle());
                        this.map.infoWindow.anchor = "ANCHOR_LOWERRIGHT ";       //固定在右上角
                        this.map.infoWindow.show(geo.geometry.getCentroid(), this.map.getInfoWindowAnchor(geo.geometry.getCentroid() ));
                        //重新设置定位位置，位置较重心上移250，使图形和弹窗均显示在地图正中央

                        var pt = new Point(geo.geometry.getCentroid().x, geo.geometry.getCentroid().y - 250, this.map.spatialReference);
      ...
      _zoomToGraphics: function(features){
        ...
        if(features && features.length > 0){
          ...
          for (var i = 0; i < length; i ++){
            ...
            var attr = features[i].attributes["OBJECTID"];
              var kg = features[i].attributes["KGSJ"];
              var jg = features[i].attributes["JGSJ"];
              var cr = features[i].attributes["CLSJ"];
            ...
            selectedGeoArray[i] = features[i];
          }
            myFeatureTable.filterRecordsByIds(arr);
